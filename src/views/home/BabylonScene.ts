import {
  ...
  export class BabylonScene {
    ...
    depth: number;
    box: Mesh;
    matPath: string = "http://localhost:3000/src/assets/images/mat1.png";
    boxMaterial;
    newMesh;
        ...
        switch (pointerInfo.type) {
          case BABYLON.PointerEventTypes.POINTERDOWN:
            if (
              ...
              pointerInfo.pickInfo!.pickedMesh?.name == "poly"
            ) {
    ...
    //创建初始板材
    createInitialBox(width: number, height: number, depth: number) {
    ...
    updateVertex(mesh: Mesh, size, direction) {
      ...
      vertices.forEach((v, n) => v.toArray(positions, n * 3));
      this.editBox.updateVerticesData(BABYLON.VertexBuffer.PositionKind, positions);
    }
    ...
    //创建槽
    createHole(width, depth, height, mesh) {
      this.holeMesh = mesh;
